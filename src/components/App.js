...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if(networkData) {
      const social = web3.eth.Contract(Social.abi, networkData.address)
      this.setState({ social })
      
      const postCount = await social.methods.postCount().call()
  ...
  createPost(content, authorName) {
    this.setState({ loading: true })
    this.state.social.methods.createPost(content, authorName).send({ from: this.state.account })
